...
class SiteSchemaOut(ModelSchema):
    class Meta:
        model = Site
        fields = ['id', 'external_id', 'name', 'study']
...
class SubjectSchemaOut(ModelSchema):
    class Meta:
        model = Subject
        fields = ['id', 'external_id', 'name', 'site']
...
class VariableSchemaOut(ModelSchema):
    class Meta:
        model = Variable
        fields = ['id', 'external_id', 'name', 'form']
...
class VisitSchemaOut(ModelSchema):
    class Meta:
        model = Visit
        fields = ['id', 'external_id', 'subject', 'interval']
...
class RecordSchemaOut(ModelSchema):
    class Meta:
        model = Record
        fields = ['id', 'external_id', 'value', 'visit', 'variable']
...
class CodingSchemaOut(ModelSchema):
    class Meta:
        model = Coding
        fields = ['id', 'external_id', 'code', 'record']
...
class QuerySchemaOut(ModelSchema):
    class Meta:
        model = Query
        fields = ['id', 'external_id', 'text', 'record']
...
class RevisionSchemaOut(ModelSchema):
    class Meta:
        model = Revision
        fields = ['id', 'external_id', 'value', 'record']
...
def sync_site(request, payload: SiteSchemaIn):
    ...
    site, _ = Site.objects.update_or_create(
        external_id=payload.external_id,
        defaults={"study": study, "name": payload.name}
    )
    return site
...
def sync_subject(request, payload: SubjectSchemaIn):
    ...
    subject, _ = Subject.objects.update_or_create(
        external_id=payload.external_id,
        defaults={"site": site, "name": payload.name}
    )
    return subject
...
def sync_variable(request, payload: VariableSchemaIn):
    ...
    variable, _ = Variable.objects.update_or_create(
        external_id=payload.external_id,
        defaults={"form": form, "name": payload.name}
    )
    return variable
...
def sync_visit(request, payload: VisitSchemaIn):
    ...
    visit, _ = Visit.objects.update_or_create(
        external_id=payload.external_id,
        defaults={"subject": subject, "interval": interval}
    )
    return visit
...
def sync_record(request, payload: RecordSchemaIn):
    ...
    record, _ = Record.objects.update_or_create(
        external_id=payload.external_id,
        defaults={"visit": visit, "variable": variable, "value": payload.value}
    )
    return record
...
def sync_coding(request, payload: CodingSchemaIn):
    ...
    coding, _ = Coding.objects.update_or_create(
        external_id=payload.external_id,
        defaults={"record": record, "code": payload.code}
    )
    return coding
...
def sync_query(request, payload: QuerySchemaIn):
    ...
    query, _ = Query.objects.update_or_create(
        external_id=payload.external_id,
        defaults={"record": record, "text": payload.text}
    )
    return query
...
def sync_revision(request, payload: RevisionSchemaIn):
    ...
    revision, _ = Revision.objects.update_or_create(
        external_id=payload.external_id,
        defaults={"record": record, "value": payload.value}
    )
    return revision
