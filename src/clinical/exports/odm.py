...
from clinical.models import Study, Subject, Record, RecordRevision, Site, Form, Variable, Interval, ExportJob
...
def create_odm_xml(study, job, zip_file_path):
    with tempfile.NamedTemporaryFile(delete=False, suffix=".xml", mode="w", encoding="utf-8") as tmp:
        ...
        for interval in intervals:
            tmp.write(f'        <StudyEventRef StudyEventOID="{escape_xml(interval.external_id)}" OrderNumber="1" Mandatory="Yes"/>\n')
        tmp.write('      </Protocol>\n')
        
        for interval in intervals:
            tmp.write(f'      <StudyEventDef OID="{escape_xml(interval.external_id)}" Name="{escape_xml(interval.name)}" Repeating="No" Type="Scheduled">\n')
            for form in Form.objects.filter(study=study):
                tmp.write(f'        <FormRef FormOID="{escape_xml(form.external_id)}" OrderNumber="1" Mandatory="Yes"/>\n')
            tmp.write('      </StudyEventDef>\n')
            
        for form in Form.objects.filter(study=study):
            tmp.write(f'      <FormDef OID="{escape_xml(form.external_id)}" Name="{escape_xml(form.name)}" Repeating="No">\n')
            tmp.write(f'        <ItemGroupRef ItemGroupOID="IG_{escape_xml(form.external_id)}" OrderNumber="1" Mandatory="Yes"/>\n')
            tmp.write('      </FormDef>\n')
            tmp.write(f'      <ItemGroupDef OID="IG_{escape_xml(form.external_id)}" Name="{escape_xml(form.name)} Group" Repeating="No">\n')
            variables = Variable.objects.filter(form=form)
            for var in variables:
                tmp.write(f'        <ItemRef ItemOID="{escape_xml(var.external_id)}" OrderNumber="1" Mandatory="Yes"/>\n')
            tmp.write('      </ItemGroupDef>\n')
            for var in variables:
                tmp.write(f'      <ItemDef OID="{escape_xml(var.external_id)}" Name="{escape_xml(var.name)}" DataType="text"/>\n')
        ...
        for site in Site.objects.filter(study=study):
            tmp.write(f'    <Location OID="{escape_xml(site.external_id)}" Name="{escape_xml(site.name)}" LocationType="Site"/>\n')
        ...
        for subject in subjects:
            ...
            records = Record.objects.filter(visit__subject=subject).select_related('visit__interval', 'variable__form').order_by(
                'source_sequence', 'clinical_timestamp', 'created_at'
            ...
            for rec in records:
                ...
                if event_oid != curr_event:
                    if curr_ig: tmp.write('          </ItemGroupData>\n')
                    if curr_form: tmp.write('        </FormData>\n')
                    if curr_event: tmp.write('      </StudyEventData>\n')
                ...
                tmp.write(f'            <ItemData ItemOID="{escape_xml(rec.variable.external_id)}" Value="{escape_xml(rec.value)}">\n')
                
                logs = AuditLog.objects.filter(model_name="Record", object_id=str(rec.external_id)).order_by('timestamp')
                for log in logs:
