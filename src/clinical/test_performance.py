...
def auth_headers(db):
    ...
    return {"HTTP_AUTHORIZATION": f"Bearer {token}", "HTTP_X_PROVIDER": str(provider.id), "HTTP_STUDYKEY": "test-study"}
...
def test_pagination_limits(client, auth_headers):
    ...
    subjects = [Subject(external_id=f"sub-{i}", site=site, name=f"Subject {i}", provider=provider) for i in range(150)]
