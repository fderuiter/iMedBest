...
import pytest
from .models import Record, Subject, Visit, SyncStatus, Query
from .tasks import poll_imednet_high_priority
...
def test_longitudinal_reconstruction(client):
    ...
    client.post(
        "/api/clinical/studies",
        data={"external_id": "study-2", "name": "Study 2"},
        content_type="application/json",
        HTTP_AUTHORIZATION="Bearer test_token",
    )
    client.post(
        "/api/clinical/sites",
        data={"external_id": "site-2", "study_ext_id": "study-2", "name": "Site 2"},
        content_type="application/json",
        HTTP_AUTHORIZATION="Bearer test_token",
    )
    client.post(
        "/api/clinical/subjects",
        data={"external_id": "sub-2", "site_ext_id": "site-2", "name": "Subject 2"},
        content_type="application/json",
        HTTP_AUTHORIZATION="Bearer test_token",
    )
    client.post(
        "/api/clinical/intervals",
        data={"external_id": "int-2", "study_ext_id": "study-2", "name": "Interval 2"},
        content_type="application/json",
        HTTP_AUTHORIZATION="Bearer test_token",
    )
    client.post(
        "/api/clinical/forms",
        data={"external_id": "form-2", "study_ext_id": "study-2", "name": "Form 2"},
        content_type="application/json",
        HTTP_AUTHORIZATION="Bearer test_token",
    )
    client.post(
        "/api/clinical/variables",
        data={"external_id": "var-2", "form_ext_id": "form-2", "name": "Variable 2"},
        content_type="application/json",
        HTTP_AUTHORIZATION="Bearer test_token",
    )
...
@pytest.mark.django_db
def test_poll_imednet_high_priority(requests_mock):
    # Setup prerequisite data
    from .models import Study, Site, Interval, Form, Variable

    Study.objects.create(external_id="study-1", name="Study 1")
    Site.objects.create(external_id="site-1", study_id=1, name="Site 1")
    Subject.objects.create(external_id="sub-1", site_id=1, name="Subject 1")
    Interval.objects.create(external_id="int-1", study_id=1, name="Interval 1")
    Form.objects.create(external_id="form-1", study_id=1, name="Form 1")
    Variable.objects.create(external_id="var-1", form_id=1, name="Variable 1")
    Visit.objects.create(external_id="visit-1", subject_id=1, interval_id=1)
    Record.objects.create(external_id="rec-1", visit_id=1, variable_id=1, value="120/80")

    # Mock iMednet API responses
    requests_mock.get(
        "http://localhost:8080/imednet_mock/high-priority/queries",
        json=[
            {
                "external_id": "query-1",
                "record_ext_id": "rec-1",
    ...
    requests_mock.get(
        "http://localhost:8080/imednet_mock/high-priority/records",
        json=[
            {
                "external_id": "rec-1",
                "visit_ext_id": "visit-1",
                "variable_ext_id": "var-1",
                "value": "120/82",  # updated value
                "clinical_timestamp": "2024-01-01T11:00:00Z",
                "source_sequence": 2,
            }
        ],
    )

    poll_imednet_high_priority()

    # Check status
    status = SyncStatus.objects.get(id=1)
    assert status.status == "SUCCESS"
    assert status.last_successful_pull is not None

    # Check query ingestion
    q = Query.objects.get(external_id="query-1")
    assert q.text == "Please confirm value"
    assert q.record.external_id == "rec-1"

    # Check record update
    r = Record.objects.get(external_id="rec-1")
    assert r.value == "120/82"


@pytest.mark.django_db
def test_poll_imednet_high_priority_no_data(requests_mock):
    requests_mock.get("http://localhost:8080/imednet_mock/high-priority/queries", status_code=404)
    requests_mock.get("http://localhost:8080/imednet_mock/high-priority/records", status_code=404)

    poll_imednet_high_priority()

    # Worker shouldn't crash, status should be SUCCESS (polling was successful even if no data found)
    status = SyncStatus.objects.get(id=1)
    assert status.status == "SUCCESS"
    assert status.last_successful_pull is not None
