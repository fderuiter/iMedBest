import pytest
from clinical.models import Provider, Study, Site, Subject as ClinicalSubject
from core.models import Subject as CoreSubject, Form, Interval, Visit, Record, Variable, RecordKeyword
...
def test_sync_records_success():
    ...
    interval = Interval.objects.create(imednet_id="int-1", interval_name="Interval 1", study=study, interval_sequence=1, interval_group_id=1, timeline="Scheduled")
...
@pytest.mark.django_db
def test_record_variable_validation_warning(caplog):
    # Setup
    provider = Provider.objects.create(name="iMednet Provider")
    study = Study.objects.create(external_id="study-123", name="Test Study", provider=provider)
    site = Site.objects.create(external_id="site-1", name="Site 1", study=study, provider=provider)
    ...
    Variable.objects.create(study=study, form=form, imednet_id="v1", variable_name="known_var", sequence=1, revision=1, variable_oid="V1")
