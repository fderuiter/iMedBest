...
def test_subject_in_extra_fields():
    ...
    with pytest.raises(ValidationError):
        SubjectIn(subject_status="Active", illegal_field="should fail")
...
def test_subject_in_read_only_fields():
    ...
    with pytest.raises(ValidationError):
        SubjectIn(subject_status="Active", imednet_id="EXT123")
...
def test_resilient_date_parsing():
    ...
    obj = DateModel(**data)
    assert obj.my_date == datetime(2023, 10, 27, 14, 30, 0, tzinfo=UTC)

    # Standard ISO string should also work if Pydantic supports it
    data2 = {"my_date": "2023-10-27T14:30:00Z"}
    obj2 = DateModel(**data2)
    assert obj2.my_date == datetime(2023, 10, 27, 14, 30, 0, tzinfo=UTC)
...
def test_subject_out_serialization(db):
    ...
    Site.objects.create(name="Test Site", study=study, provider=provider, external_id="SITE1")
