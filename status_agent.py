...
def log_status(service, status, details=""):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("INSERT INTO state_history (service, status, details) VALUES (?, ?, ?)",
...
def log_resources(cpu, mem, disk):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("INSERT INTO resource_history (cpu_percent, memory_mb, disk_percent) VALUES (?, ?, ?)",
              (cpu, mem, disk))
    conn.commit()
    conn.close()
...
def notify_webhook(service, status, details):
    if not WEBHOOK_URL:
        return
    try:
        data = json.dumps({"service": service, "status": status, "details": details}).encode("utf-8")
        ...
        if self.path == "/":
            ...
            try:
                ...
                for row in latest_status:
                    ...
                    status_html += f"<div><h3>{svc.upper()}</h3><p>Status: <strong style='color:{color}'>{status}</strong></p><p>Last update: {ts}</p><p>Details: {details}</p></div>"
...
def run_server():
    with ThreadingTCPServer(("", PORT), StatusHandler) as httpd:
        print(f"Serving status dashboard at port {PORT}")
        httpd.serve_forever()
